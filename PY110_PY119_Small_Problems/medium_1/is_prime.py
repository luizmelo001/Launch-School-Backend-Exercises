"""
A prime number is a positive number that is evenly divisible only by itself and 1. Thus, 23 is prime since its only divisors are 1 and 23.
...
You may not use any of Python's add-on packages to solve this problem. Your task is to programmatically determine whether a number 
is prime without relying on functions that already do that for you.

Algorithm
    -Define a range from 1 up to the given number
    -Perform division and check if the remainder is 0
    -If reaminder is zero after 1 and before the number, return False, since it's not prime
    -Else, return True
"""
def is_prime(num):
    if num == 1:
